...
public class EmployerUpdateStatusManager implements EmployerUpdateStatusService{
	...
	public DataResult<List<EmployerUpdateStatus>> getAll() {
		return new SuccessDataResult<List<EmployerUpdateStatus>>
		(this.employerUpdateStatusDao.findAll(),"başarılı");
	}

	@Override
	public DataResult<EmployerUpdateStatus> findByEmployerId(int id) {
		return new SuccessDataResult<EmployerUpdateStatus>
		(this.employerUpdateStatusDao.findByEmployerId(id),"başarılı");
	...
	public Result confirm(int id) {
		...
		if(newDataRef != null) {
			...
			Result engine = Injection.run(existsByEmployerId(id),updateChecker(ref,newDataRef));
			if(!engine.isSuccess()) {
	...
	public Result add(EmployerUpdateStatus employer) {
		if(this.employerUpdateStatusDao.existsByEmployerId(employer.getEmployerId())) {
			return new ErrorResult("Bekleyen güncelleme isteği var");
		}
		else {
